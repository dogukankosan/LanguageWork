...
{
    public partial class SpianImageAddDeleteUpdate : DevExpress.XtraEditors.XtraForm
    {
        public SpianImageAddDeleteUpdate()
        {
            InitializeComponent();
        }
        ...
        private void Add()
        {
            if (DatabaseControl.sqliteDatabaseControlImage(Pct_Question, Txt_A, Txt_B, Txt_C, Txt_D, Cmb_True))
            {
                ...
                {
                    if (SQLITEDBBase.SqlLiteDatabaseAdd("insert into SpainImageQuestion(ImageQuestions,A,B,C,D,TrueAnswer) values('" + ImageConvert.ImageToBase64(Pct_Question.Image, ImageFormat.Jpeg) + "', '" + Txt_A.Text.Trim().ToUpper() + "', '" + Txt_B.Text.Trim().ToUpper() + "', '" + Txt_C.Text.Trim().ToUpper() + "', '" + Txt_D.Text.Trim().ToUpper() + "', '" + trueAnswer.Trim().ToUpper() + "')"))
                    {
                        ...
                        gridControl1.DataSource = SQLITEDBBase.QuestionGrid(
                            "select ID,ImageQuestions AS 'SORU',A,B,C,D,TrueAnswer as 'DOGRUCEVAP' from SpainImageQuestion");
                    }
                }
                else
        ...
        private void UpdateWord()
        {
            if (!string.IsNullOrEmpty(id))
            {
                if (DatabaseControl.sqliteDatabaseControlImage(Pct_Question, Txt_A, Txt_B, Txt_C, Txt_D, Cmb_True))
                {
                    ...
                    {
                        TrueAnswerSelect();
                        if (SQLITEDBBase.SqlLiteDatabaseUpdate("update SpainImageQuestion set ImageQuestions='" + ImageConvert.ImageToBase64(Pct_Question.Image, ImageFormat.Jpeg) + "',A='" + Txt_A.Text.Trim().ToUpper() + "',B='" + Txt_B.Text.Trim().ToUpper() + "',C='" + Txt_C.Text.Trim().ToUpper() + "',D='" + Txt_D.Text.Trim().ToUpper() + "',TrueAnswer='" + trueAnswer + "' where ID=" + id + ""))
                        {
        ...
        private void Delete()
        {
            if (!string.IsNullOrEmpty(id))
            {
                if (XtraMessageBox.Show("SORU SİLMEK İSTEDİĞİNİZDEN EMİN MİSİNİZ ?", "SİLME İŞLEMİ",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (SQLITEDBBase.SqLiteDatabaseDelete("delete from SpainImageQuestion where ID=" + id + ""))
                    {
        ...
        private void SpianImageAddDeleteUpdate_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void Btn_Clear_MouseHover(object sender, System.EventArgs e)
        {
        ...
        private void Txt_A_EditValueChanged(object sender, EventArgs e)
        {
            Txt_A.Text = DatabaseControl.turkishCharterReplace(Txt_A);
        }
        private void Txt_B_EditValueChanged(object sender, EventArgs e)
        {
            Txt_B.Text = DatabaseControl.turkishCharterReplace(Txt_B);
        }
        private void Txt_C_EditValueChanged(object sender, EventArgs e)
        {
            Txt_C.Text = DatabaseControl.turkishCharterReplace(Txt_C);
        }
        private void Txt_D_EditValueChanged(object sender, EventArgs e)
        {
            Txt_D.Text = DatabaseControl.turkishCharterReplace(Txt_D);
        }
        private void SpianImageAddDeleteUpdate_Load(object sender, System.EventArgs e)
        {
            gridControl1.DataSource = SQLITEDBBase.QuestionGrid(
                "select ID,ImageQuestions AS 'SORU',A,B,C,D,TrueAnswer as 'DOGRUCEVAP' from SpainImageQuestion");
            try
