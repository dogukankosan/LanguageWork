...
{
    public partial class SpainQuestionImageExam : DevExpress.XtraEditors.XtraForm
    {
        public SpainQuestionImageExam()
        {
            InitializeComponent();
        }
        ...
        {
            if (SQLITEDBBase.QuestionImageGet(pictureBox1, Lbl_TrueAnswer, Btn_A, Btn_B, Btn_C, Btn_D, Lbl_QuestionCount, "select count(ID) from SpainImageQuestion", "select ImageQuestions,TrueAnswer,A,B,C,D from SpainImageQuestion where ID=0") == false)
                return;
            TimerVariable();
            formClosed = true;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (timerValue == 0)
            {
                ...
                formClosed = false;
                timerValue = default;
                SQLITEDBBase.SqlLiteDatabaseAdd("insert into SpainResultImage (TrueCount,FalseCount,_Date) values (" + Lbl_TrueCount.Text + "," + Lbl_FalseCount.Text + ",'" + DateTime.Now.ToString("d") + "') ");
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (timerSleep == 0)
            {
                ...
                BtnEnabled(true);
                SQLITEDBBase.QuestionImageGet(pictureBox1, Lbl_TrueAnswer, Btn_A, Btn_B, Btn_C, Btn_D, Lbl_QuestionCount, "select count(ID) from SpainImageQuestion", "select ImageQuestions,TrueAnswer,A,B,C,D from SpainImageQuestion where ID=0");
                timer1.Start();
            }
        }
        private void SpainQuestionImageExam_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = formClosed;
        }
        private void SpainQuestionImageExam_Load(object sender, EventArgs e)
        {
