...
{
    public partial class EnglishWordQuestionAddDeleteUpdate : DevExpress.XtraEditors.XtraForm
    {
        public EnglishWordQuestionAddDeleteUpdate()
        {
            InitializeComponent();
        }
        ...
        private void Add()
        {
            if (DatabaseControl.sqliteDatabaseControl(Txt_Question, Txt_A, Txt_B, Txt_C, Txt_D, Cmb_True))
            {
                ...
                {
                    if (SQLITEDBBase.SqlLiteDatabaseAdd("insert into EnglishWord(Question, AnswerA, AnswerB, AnswerC, AnswerD, TrueAnswer) values('" + Txt_Question.Text.Trim().ToUpper() + "', '" + Txt_A.Text.Trim().ToUpper() + "', '" + Txt_B.Text.Trim().ToUpper() + "', '" + Txt_C.Text.Trim().ToUpper() + "', '" + Txt_D.Text.Trim().ToUpper() + "', '" + trueAnswer.Trim().ToUpper() + "')"))
                    {
                        ...
                        gridControl1.DataSource = SQLITEDBBase.QuestionGrid(
                            "select ID,Question AS 'SORU',AnswerA AS 'A',AnswerB AS 'B',AnswerC AS 'C',AnswerD AS 'D',TrueAnswer as 'DOGRUCEVAP' from EnglishWord");
                    }
                }
                else
        ...
        private void UpdateWord()
        {
            if (!string.IsNullOrEmpty(id))
            {
                if (DatabaseControl.sqliteDatabaseControl(Txt_Question, Txt_A, Txt_B, Txt_C, Txt_D, Cmb_True))
                {
                    ...
                    {
                        TrueAnswerSelect();
                        if (SQLITEDBBase.SqlLiteDatabaseUpdate("update EnglishWord set Question='" + Txt_Question.Text.Trim().ToUpper() + "',AnswerA='" + Txt_A.Text.Trim().ToUpper() + "',AnswerB='" + Txt_B.Text.Trim().ToUpper() + "',AnswerC='" + Txt_C.Text.Trim().ToUpper() + "',AnswerD='" + Txt_D.Text.Trim().ToUpper() + "',TrueAnswer='" + trueAnswer + "' where ID=" + id + ""))
                        {
        ...
        private void Delete()
        {
            if (!string.IsNullOrEmpty(id))
            {
                if (XtraMessageBox.Show("SORU SİLMEK İSTEDİĞİNİZDEN EMİN MİSİNİZ ?", "SİLME İŞLEMİ",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (SQLITEDBBase.SqLiteDatabaseDelete("delete from EnglishWord where ID=" + id + ""))
                    {
        ...
        private void QuestionAdd_Load(object sender, System.EventArgs e)
        {
            Txt_Question.Focus();
            gridControl1.DataSource = SQLITEDBBase.QuestionGrid(
                "select ID,Question AS 'SORU',AnswerA AS 'A',AnswerB AS 'B',AnswerC AS 'C',AnswerD AS 'D',TrueAnswer as 'DOGRUCEVAP' from EnglishWord");
        ...
        private void QuestionAdd_KeyDown(object sender, KeyEventArgs e)
        {
