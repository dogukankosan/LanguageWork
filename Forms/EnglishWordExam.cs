...
{
    public partial class EnglishWordExam : DevExpress.XtraEditors.XtraForm
    {
        public EnglishWordExam()
        {
            InitializeComponent();
        }
        ...
        {
            if (SQLITEDBBase.QuestionGet(Lbl_Question, Lbl_TrueAnswer, Btn_A, Btn_B, Btn_C, Btn_D, Lbl_QuestionCount, "select count(ID) from EnglishWord", "select Question,TrueAnswer,AnswerA,AnswerB,AnswerC,AnswerD from EnglishWord where ID=0") == false)
                return;
            TimerVariable();
            formClosed = true;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (timerValue == 0)
            {
                ...
                formClosed = false;
                timerValue = default;
                SQLITEDBBase.SqlLiteDatabaseAdd("insert into EnglishResultWord (TrueCount,FalseCount,_Date) values (" + Lbl_TrueCount.Text + "," + Lbl_FalseCount.Text + ",'" + DateTime.Now.ToString("d") + "') ");
        ...
        {
            toolTip1.SetToolTip(Btn_Voice, "SORUYU İNGİLİZCE OLARAK OKU");
        }
        private void Exam_FormClosing(object sender, System.Windows.Forms.FormClosingEventArgs e)
        {
            e.Cancel = formClosed;
        }
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (timerSleep == 0)
            {
                ...
                BtnEnabled(true);
                SQLITEDBBase.QuestionGet(Lbl_Question, Lbl_TrueAnswer, Btn_A, Btn_B, Btn_C, Btn_D, Lbl_QuestionCount, "select count(ID) from EnglishWord", "select Question,TrueAnswer,AnswerA,AnswerB,AnswerC,AnswerD from EnglishWord where ID=0");
                timer1.Start();
            }
        }
    }
