...
{
    public partial class SpainWordExam : DevExpress.XtraEditors.XtraForm
    {
        public SpainWordExam()
        {
            InitializeComponent();
        }
        ...
        {
            if (SQLITEDBBase.QuestionGet(Lbl_Question, Lbl_TrueAnswer, Btn_A, Btn_B, Btn_C, Btn_D, Lbl_QuestionCount, "select count(ID) from SpainWord", "select Question,TrueAnswer,AnswerA,AnswerB,AnswerC,AnswerD from SpainWord where ID=0") == false)
                return;
            TimerVariable();
            formClosed = true;
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (timerSleep == 0)
            {
                ...
                BtnEnabled(true);
                SQLITEDBBase.QuestionGet(Lbl_Question, Lbl_TrueAnswer, Btn_A, Btn_B, Btn_C, Btn_D, Lbl_QuestionCount, "select count(ID) from SpainWord", "select Question,TrueAnswer,AnswerA,AnswerB,AnswerC,AnswerD from SpainWord where ID=0");
                timer1.Start();
            }
        }
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (timerValue == 0)
            {
                ...
                formClosed = false;
                timerValue = default;
                SQLITEDBBase.SqlLiteDatabaseAdd("insert into SpainResultWord (TrueCount,FalseCount,_Date) values (" + Lbl_TrueCount.Text + "," + Lbl_FalseCount.Text + ",'" + DateTime.Now.ToString("d") + "') ");
                XtraMessageBox.Show("SÜRENİZ BİTMİŞTİR KELİME ÇALIŞMASI TAMAMLANMIŞTIR !!", "SÜRE TAMAMLANDI", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            timerValue--;
        }
        private void SpainWordExam_Load(object sender, EventArgs e)
        {
            axWindowsMediaPlayer1.Visible = false;
            BtnEnabled(false);
            TimerVariable();
        }
        private void Btn_Voice_MouseHover(object sender, EventArgs e)
        {
            toolTip1.SetToolTip(Btn_Voice, "SORUYU İSPANYOLCA OLARAK OKU");
        }
        private void SpainWordExam_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = formClosed;
        }
        private void Btn_Voice_Click(object sender, EventArgs e)
        {
            ...
            ss.SpeakAsync(Lbl_Question.Text);
        }
    }
}